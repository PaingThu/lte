import{a as e,c as t,d as n,f as r,i,n as a,o,r as s,u as c}from"../config-BxYaVZLE.js";import{n as l,t as u}from"../api-B2cwr0sz.js";import{t as d}from"../flag-icons.min-mpiiwlBd.js";var f=r((async()=>{await e();var t=document.querySelector(`#logo-img`);t.src=c;var n={logoutBtn:document.getElementById(`logoutBtn`)};function r(){console.log(`Logging out...`,a),document.cookie=`userToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`,window.location.href=i()+`admin/login/`}n.logoutBtn.addEventListener(`click`,r)})),p=r((async()=>{await e(),d();var t=window.location.pathname,n=i().replace(/\/$/,``),r=document.getElementById(`englishSection`);t===`${n}/admin/`?r.removeAttribute(`open`):r.setAttribute(`open`,``)})),m=r((async()=>{await l(),await e();var n=document.getElementById(`displayArea`),r=document.getElementById(`addExampleBtn`),a=document.getElementById(`exampleSentencesContainer`),c=document.getElementById(`patternForm`);document.getElementById(`submitBtn`);var d=document.getElementById(`btnText`),f=document.getElementById(`btnLoader`),p=document.getElementById(`newPatternBtn`),m=document.getElementById(`patternFormContainer`),h=document.getElementById(`patternListContainer`),g=1,_=[];s();async function v(){try{let e=await u.getLevels();console.log(`Levels: `,e),_=e.map(e=>({id:e[0],value:e[1],label_en:e[2],label_ja:e[3],label_my:e[4]}));let t=document.getElementById(`level`);t.innerHTML=``,_.forEach(e=>{let n=document.createElement(`option`);n.value=e.id,n.textContent=e.label_en,t.appendChild(n)})}catch(e){console.error(`Error fetching levels: `,e)}}v(),r.addEventListener(`click`,()=>{g++;let e=document.createElement(`textarea`);e.id=`exampleSentence${g}`,e.required=!0,e.className=`example-sentence w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-primary-light focus:border-transparent outline-none transition`,e.placeholder=`Enter example sentence ${g}`;let t=document.createElement(`button`);t.type=`button`,t.innerHTML=`<svg 
                ...
                >
                    ...
                    />
                </svg>`,t.className=`absolute right-3 cursor-pointer bg-gray-500 hover:bg-gray-600 text-white font-semibold py-1 px-3 rounded-lg transition shadow-md hover:shadow-lg w-max`,t.addEventListener(`click`,()=>{a.removeChild(e.parentElement)});let n=document.createElement(`div`);n.className=`relative flex items-center gap-2`,n.appendChild(e),n.appendChild(t),a.appendChild(n)}),c.addEventListener(`submit`,async e=>{e.preventDefault(),d.textContent=`Submitting...`,f.classList.remove(`hidden`);let t=document.getElementById(`level`).value,r=document.getElementById(`title`).value,i=document.getElementById(`formula`).value,o=document.getElementById(`explanationEn`).value,s=document.getElementById(`explanationJa`).value,l=document.getElementById(`explanationMy`).value,p=[];for(let e=1;e<=g;e++){let t=document.getElementById(`exampleSentence${e}`);t&&p.push(t.value)}let m={level:t,title:r,formula:i,explanationEn:o,explanationJa:s,explanationMy:l,exampleSentences:p},h=await u.savePattern(m);d.textContent=`Create Pattern`,f.classList.add(`hidden`),h.status===`ok`?(n.innerHTML=`<p class="text-green-500 p-3">Pattern saved successfully!</p>`,a.innerHTML=`
        ...
        `;e.forEach(e=>{JSON.stringify(e.examples).replace(/"/g,`&quot;`),r+=`
                <tr>
                    ...
                    <td class="p-2 border text-center">
                        <a href="${i()}admin/english-pattern/detail/?id=${e.id}" class="ml-2 text-blue-500 hover:underline">></a>
                    </td>
